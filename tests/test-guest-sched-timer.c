...
extern uint8_t guest_timer1_image_end[];

static const GuestImage guest_images[] = {
    { 0, GUEST_SLOT_BASE(0), guest_timer0_image_start, guest_timer0_image_end },
    { 1, GUEST_SLOT_BASE(1), guest_timer1_image_start, guest_timer1_image_end },
};
...
static void init_guest_vcpus(void) {
    ...
    uint64_t period = USEC_PER_SEC / GUEST_TIMER_HZ;

    for (uint32_t i = 0; i < sizeof(guest_images) / sizeof(guest_images[0]); i++) {
        const GuestImage *image = &guest_images[i];
        HvVcpu *vcpu = &scheduler.vcpus[image->id];
