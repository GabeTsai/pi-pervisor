...
extern uint8_t guest0_stage2_fault_image_start[];
extern uint8_t guest0_stage2_fault_image_end[];
extern uint8_t guest1_stage2_fault_image_start[];
extern uint8_t guest1_stage2_fault_image_end[];

static const GuestImage guest_images[] = {
    { 0, GUEST_SLOT_BASE(0), guest0_stage2_fault_image_start, guest0_stage2_fault_image_end },
    { 1, GUEST_SLOT_BASE(1), guest1_stage2_fault_image_start, guest1_stage2_fault_image_end },
};
...
static void init_guest_vcpus(void) {
    for (uint32_t i = 0; i < sizeof(guest_images) / sizeof(guest_images[0]); i++) {
        ...
        hv_vcpu_init(&scheduler.vcpus[image->id],
                     image->id,
                     GUEST_BASE,
                     0, // linker script set stack, dummy value
                     HYP_LOWER_SVC_CPSR);
    }
